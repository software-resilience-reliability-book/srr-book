...
This is why we refer to them as contracts. If we pass in an `RgbColor` to a function, we can guarantee that that it will have a `Red`, `Green`, and `Blue` property.
